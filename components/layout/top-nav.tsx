...
import { Search, Moon, Sun, ChevronRight, Home, Crown } from "lucide-react"
...
export function TopNav() {
  ...
  return (
    <header className={`sticky top-0 z-10 flex h-12 sm:h-14 items-center gap-2 sm:gap-4 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 ${PAGE_PADDING.horizontal}`}>
      ...
      <div className="hidden sm:flex flex-1 max-w-md ml-auto md:ml-0 md:mx-auto">
        <form onSubmit={handleSearch} className="relative w-full">
          ...
          <Input
            ...
            className="pl-9 h-9 bg-muted/50 border-border w-full"
          />
        </form>
      </div>
