...
export function TikTokResultCard({ video, rank, onCopy }: TikTokResultCardProps) {
  return (
    <div className="rounded-xl border border-border bg-card p-2.5 sm:p-4 hover:border-cyan-500/30 transition-colors group overflow-hidden">
      {/* Mobile Layout */}
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex flex-col items-center gap-1 shrink-0">
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-cyan-500/10 flex items-center justify-center text-[10px] sm:text-sm font-bold text-cyan-500">
              {rank}
            </div>
            <div className={cn(
          ...
          <div className="flex items-center gap-0.5 shrink-0">
            <Button
              ...
              onClick={() => onCopy(video.description)}
            >
              <CopyIcon size={12} />
            </Button>
            <Button
        ...
        <div className="grid grid-cols-4 gap-1 sm:gap-2">
          ...
          <div className="text-center p-1 sm:p-1.5 rounded-md bg-muted/30">
            <p className="font-bold text-foreground text-[10px] sm:text-xs">{formatViews(video.shares)}</p>
            <p className="text-[8px] sm:text-[10px] text-muted-foreground">share</p>
          </div>
        ...
        <div className="flex gap-1 flex-wrap">
          {video.hashtags.slice(0, 3).map((tag) => (
            <Badge
              key={tag}
              variant="secondary"
              className="text-[9px] sm:text-[10px] px-1.5 py-0 bg-cyan-500/10 text-cyan-500 cursor-pointer"
              onClick={() => onCopy(`#${tag}`)}
            >
              #{tag}
            </Badge>
