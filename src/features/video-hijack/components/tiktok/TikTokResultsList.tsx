...
import { TikTokResultCard } from "./TikTokResultCard"
...
export function TikTokResultsList({ results, isLoading, searchQuery }: TikTokResultsListProps) {
  const [sortBy, setSortBy] = useState<SortOption>("opportunity")
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid")
  const [selectedVideo, setSelectedVideo] = useState<TikTokVideoResult | null>(null)
  ...
  // Export to CSV
  const handleExport = () => {
    tiktokService.exportToCSV(sortedResults, `tiktok-${searchQuery}-${Date.now()}`)
  }

  if (results.length === 0 && !isLoading) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-card/50 p-8 sm:p-12 text-center">
        <TikTokIcon size={32} className="mx-auto text-[#ff0050]/50 sm:w-12 sm:h-12" />
        <h3 className="mt-4 text-sm sm:text-base font-semibold text-foreground">No TikTok Results</h3>
        <p className="mt-2 text-xs sm:text-sm text-muted-foreground max-w-md mx-auto">
          Search for a keyword to discover TikTok video opportunities
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="font-semibold text-sm sm:text-base text-foreground">TikTok Results</h2>
      ...
      {/* Results Grid - TikTok style uses smaller cards */}
      <div className={cn(
        viewMode === "grid"
          ? "grid grid-cols-2 xs:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-3"
          : "space-y-3"
      )}>
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-8">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#ff0050]" />
        </div>
      )}
    </div>
  )
