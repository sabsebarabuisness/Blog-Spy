...
export function BentoGrid({ quickActions, recentActivity }: BentoGridProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      ...
      <Card className="col-span-1 sm:col-span-2 bg-gradient-to-br from-card to-card/50 border-border/50 backdrop-blur-sm hover:border-border transition-all duration-300">
        <CardContent className="p-4 sm:p-6">
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-3 gap-2 sm:gap-3">
              {quickActions.map((action) => (
                <Link
                  key={action.title}
                  href={action.href}
                  className={`flex flex-col items-center justify-center gap-1.5 sm:gap-2 p-3 sm:p-4 rounded-lg sm:rounded-xl bg-gradient-to-br ${action.color} border border-border/30 hover:border-border/50 active:scale-[0.98] hover:scale-[1.02] transition-all duration-200`}
                >
